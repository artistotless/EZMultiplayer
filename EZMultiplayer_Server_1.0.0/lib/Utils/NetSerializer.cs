...
namespace NetworkLib.Utils
{
    public sealed class NetSerializer
    {
        ...
        private delegate void NestedTypeWriter(Message writer, object customObj);
        private delegate object NestedTypeReader(NetDataReader reader);

        private sealed class StructInfo
        {
            public readonly Action<Message>[] WriteDelegate;
            ...
            public StructInfo(int membersCount)
            {
                ...
                WriteDelegate = new Action<Message>[membersCount];
                ReadDelegate = new Action<NetDataReader>[membersCount];
            }

            public void Write(Message writer, object obj)
            {
                Reference = obj;
        ...
        private readonly Message _writer;
        ...
        /// <returns>True - if register successful, false - if type already registered</returns>
        public bool RegisterNestedType<T>(Action<Message, T> writeDelegate, Func<NetDataReader, T> readDelegate)
        {
            var t = typeof(T);
            ...
            for (int i = 0; i < propsCount; i++)
            {
                ...
                if (isEnum)
                {
                    ...
                    if (underlyingType == typeof(byte))
                    {
                        ...
                        {
                            writer.Add((byte)property.GetValue(info.Reference, null));
                        };
                    }
                    else if (underlyingType == typeof(int))
                    {
                        ...
                        info.WriteDelegate[i] = writer =>
                        {
                            writer.Add((int)property.GetValue(info.Reference, null));
                        };
                    }
                    else
                    {
                ...
                else if (propertyType == typeof(string))
                {
                    ...
                    if (_maxStringLength <= 0)
                    {
                        ...
                        info.WriteDelegate[i] = writer => writer.Add(getDelegate((T)info.Reference));
                    }
                    else
                    {
                        info.ReadDelegate[i] = reader => setDelegate((T)info.Reference, reader.GetString(_maxStringLength));
                        info.WriteDelegate[i] = writer => writer.Add(getDelegate((T)info.Reference), _maxStringLength);
                    }
                }
                ...
                else if (propertyType == typeof(string[]))
                {
                    ...
                    if (_maxStringLength <= 0)
                    {
                        ...
                        info.WriteDelegate[i] =
                            writer => writer.AddArray(getDelegate((T) info.Reference));
                    }
                    else
                    {
                        info.ReadDelegate[i] =
                        ...
                        info.WriteDelegate[i] =
                            writer => writer.AddArray(getDelegate((T)info.Reference), _maxStringLength);
                    }
                }
                else if (propertyType == typeof(byte[]))
                {
                    ...
                    info.WriteDelegate[i] = writer => writer.AddBytesWithLength(getDelegate((T)info.Reference));
                }
                else if (propertyType == typeof(short[]))
                {
                    var setDelegate = ExtractSetDelegate<T, short[]>(setMethod);
                    ...
                    info.WriteDelegate[i] = writer => writer.AddArray(getDelegate((T)info.Reference));
                }
                else if (propertyType == typeof(ushort[]))
                {
                    var setDelegate = ExtractSetDelegate<T, ushort[]>(setMethod);
        ...
        /// <summary>
        /// Serialize struct to Message (fast)
        /// </summary>
        /// <param name="writer">Serialization target Message</param>
        /// <param name="obj">Object to serialize</param>
        /// <exception cref="InvalidTypeException"><typeparamref name="T"/>'s fields are not supported, or it has no fields</exception>
        public void Serialize<T>(Message writer, T obj) where T : class, new()
        {
