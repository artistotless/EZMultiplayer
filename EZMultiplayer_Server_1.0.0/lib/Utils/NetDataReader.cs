...
{
    public class NetDataReader : ICloneable
    {
        ...
        public void SetSource(Message dataWriter)
        {
        ...
        {
            var hostName = GetString();
            var password = GetString();
            var nameRoom = GetString();
            var maxPlayers = GetUInt();

            return new RoomData(nameRoom, hostName, maxPlayers, password);
        }
