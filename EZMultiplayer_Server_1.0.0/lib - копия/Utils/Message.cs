...
namespace LiteNetLib.Utils
{
    public class Message
    {
        ...
        /// <summary>
        /// Creates Message from existing ByteArray
        /// </summary>
        /// <param name="bytes">Source byte array</param>
        /// <param name="copy">Copy array to new location or use existing</param>
        public static Message FromBytes(byte[] bytes, bool copy)
        {
            if (copy)
            {
                var netDataWriter = new Message(true, bytes.Length);
                netDataWriter.Put(bytes);
                return netDataWriter;
            }
            return new Message(true, 0) {_data = bytes};
        }

        /// <summary>
        /// Creates Message from existing ByteArray (always copied data)
        /// </summary>
        /// <param name="bytes">Source byte array</param>
        /// <param name="offset">Offset of array</param>
        /// <param name="length">Length of array</param>
        public static Message FromBytes(byte[] bytes, int offset, int length)
        {
            var netDataWriter = new Message(true, bytes.Length);
